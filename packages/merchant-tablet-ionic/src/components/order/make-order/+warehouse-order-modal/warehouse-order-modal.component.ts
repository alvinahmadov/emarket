...
export class WarehouseOrderModalComponent implements OnInit, OnDestroy
{
	...
	@Input()
	public showOrderAction: boolean = true;
	
	@Output()
	public makeOrderEmitter = new EventEmitter<IOrderCreateInputProduct[]>();
	...
	private _loadDataSmartTable()
	{
		...
		const productsData = this._warehouseProducts.map(
				(wp: WarehouseProduct) =>
				{
					return {
						img:       `
						<img src="${this._getTranslate(wp.product['images'])}" height="68px" alt=""/>
					`,
						product:   `
	...
	private _translate(key: string): string
	{
		...
		this._translateService.get(key).subscribe((res) => translationResult = res);
